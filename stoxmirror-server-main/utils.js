...
function logoImgTag() {
  ...
  return `<div style="text-align:center;padding:18px 0;"><img src="data:image/png;base64,${BASE64_LOGO}" alt="Imperiumstox" style="max-width:220px;height:auto;display:inline-block" /></div>`;
}
      ...
      <div style="max-width:720px;margin:24px auto;padding:0 16px;">
        <div style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 8px 30px rgba(12,20,45,0.06);">
          ...
          <div style="background:#fbfbfd;padding:16px;text-align:center;color:#7b7f88;font-size:13px;">
            © ${new Date().getFullYear()} Imperiumstox. All rights reserved.
          </div>
        </div>
      </div>
    </body>
...
async function sendWelcomeEmail({ to, otp }) {
  const htmlContent = `
    <p>Hi there 👋,</p>
    <p>Welcome to Imperiumstox — we're glad to have you on board.</p>
  ...
  `;
  return sendEmail({ to, subject: "🎉 Welcome to Imperiumstox!", html: wrap(htmlContent, "Welcome to Imperiumstox") });
}
