...
const sendWithdrawalRequestEmail = async ({ from, amount, method, address }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER || "no-reply@imperiumstox.com",
      to: "support@imperiumstox.com",
      ...
      html: `
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>Best regards,<br>imperiumstox Team</p>
              <p style="font-size: 12px; color: #888;">© ${new Date().getFullYear()} imperiumstox. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
const sendWelcomeEmail = async ({ to, otp }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER,
      to: to,
      subject: "Verify Your imperiumstox Account",
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          <meta charset="UTF-8" />
          <meta name="viewport" content="width=device-width, initial-scale=1.0" />
          <title>imperiumstox Verification</title>
          <style>
            body {
        ...
        <body>
          <div class="container">
            <div class="header">
              <img src="https://imperiumstox.com/logo.png" alt="imperiumstox Logo" />
              <h1>Welcome to imperiumstox</h1>
            </div>
            <div class="content">
              <h2>Verify Your Email</h2>
              <p>Hello,</p>
              <p>Thank you for creating an account with <strong>imperiumstox</strong>. To protect your account, please use the verification code below to complete your registration:</p>

              <div class="otp-box">
                ${otp}
              </div>

              <p>This code will expire in <strong>5 minutes</strong>. For your security, never share this code with anyone—even a imperiumstox representative.</p>

              <div class="note">
                If you didn’t sign up for a imperiumstox account, please ignore this email or contact our support team immediately.
              </div>
            </div>
            <div class="footer">
              <p>Need help? Contact our <a href="mailto:support@imperiumstox.com">support team</a>.</p>
              <p>&copy; ${new Date().getFullYear()} imperiumstox. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
const sendAdminWithdrawalNotification = async ({ userName, userEmail, amount, method, timestamp }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER,
      to: "support@imperiumstox.com", // admin email from environment variable
